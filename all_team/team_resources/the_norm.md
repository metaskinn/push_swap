...
* A typedef's name must start by `t_`. union's name must start by `u_`. 
...
> **Note:** The default standard header may not automatically be configured with your personnal information. You may need to change it to follow the previous rule.
...
* In the case of a multibinary project, in addition to the above rules, you must have a rule for each binary (eg: `$(NAME_1)`, `$(NAME_2)`, ...). The "all" rule will compile all the binaries, using each binary rule.
